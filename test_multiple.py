...
def test_multiple(generator, discriminator, opt, dataloader, scale):
    ...
    inputs = torch.FloatTensor(opt.batchSize, 3, opt.imageSize, opt.imageSize)
    high_res_fake = torch.FloatTensor(
        opt.batchSize, 3, opt.imageSize, opt.imageSize)
    max_psnr = 0.0
    mean_psnr = 0.0
    min_psnr = 999.0
    ...
    for batch_no, data in enumerate(dataloader['test']):
        ...
        if opt.cuda:
            ...
            high_res_fake = generator(Variable(inputs[0][np.newaxis, :]).cuda(),
                                      Variable(
                                          inputs[1][np.newaxis, :]).cuda(),
                                      Variable(
                                          inputs[2][np.newaxis, :]).cuda(),
        ...
        imsave(high_res_fake.cpu().data, train=False,
               epoch=batch_no, image_type='fake')
...
#                         mean_ssim += pytorch_ssim.ssim(fake_img.cpu().data, gt_img).data.item()
...
def test_angres(angres, lflists, opt):
    ...
    for lf_image in lflists:
        ...
        gt_center_img = torch.FloatTensor(
            1, 3, opt.upSampling * opt.imageSize, opt.upSampling * opt.imageSize)
        gt_horizontaltop_img = torch.FloatTensor(
            1, 3, opt.upSampling * opt.imageSize, opt.upSampling * opt.imageSize)
        gt_verticalleft_img = torch.FloatTensor(
            1, 3, opt.upSampling * opt.imageSize, opt.upSampling * opt.imageSize)
        ...
        while i < 14:
            ...
            while j < 14:
                ...
                gt_horizontaltop_img[0] = torch.transpose(
                    gt_horizontaltop, 1, 2)
                gt_verticalleft_img[0] = torch.transpose(gt_verticalleft, 1, 2)
                gt_horizontalbottom_img[0] = torch.transpose(
                    gt_horizontalbottom, 1, 2)
                gt_verticalright_img[0] = torch.transpose(
                    gt_verticalright, 1, 2)
                ...
                with torch.no_grad():
                    ...
                    if opt.cuda:
                        ...
                        center_loss = content_criterion(
                            fake_img[0], gt_center_img[0].cuda())

                        horizontaltop_loss = content_criterion(
                            fake_img[1], gt_horizontaltop_img[0].cuda())
                        horizontalbottom_loss = content_criterion(
                            fake_img[2], gt_horizontalbottom_img[0].cuda())
                        verticalleft_loss = content_criterion(
                            fake_img[3], gt_verticalleft_img[0].cuda())
                        verticalright_loss = content_criterion(
                            fake_img[4], gt_verticalright_img[0].cuda())
                        total_loss = center_loss + horizontaltop_loss + \
                            horizontalbottom_loss + verticalleft_loss + verticalright_loss

                        save_image(fake_img[0], "test.png")

                        imsave(fake_img.cpu().data, train=False,
                               epoch=count, image_type='new', ang_res=True)
                        imsave(gt_center_img, train=False, epoch=count,
                               image_type='real_center', ang_res=True)
                        imsave(gt_horizontaltop_img, train=False, epoch=count,
                               image_type='real_horizontal', ang_res=True)
                        imsave(gt_horizontalbottom_img, train=False, epoch=count, image_type='real_horizontal',
                               ang_res=True)
                        imsave(gt_verticalleft_img, train=False, epoch=count,
                               image_type='real_vertical', ang_res=True)
                        imsave(gt_verticalright_img, train=False, epoch=count,
                               image_type='real_vertical', ang_res=True)
                        imsave(new_img, train=False, epoch=count,
                               image_type='actual', ang_res=True)

                        ssim_center = pytorch_ssim.ssim(
                            fake_img[0][np.newaxis, :].cpu().data, gt_center_img)
                        psnr_center_val = angres_psnr(
                            gt_center_img.cpu().data, fake_img[0].cpu().data)

                        ssim_horizontaltop = pytorch_ssim.ssim(
                            fake_img[1][np.newaxis, :].cpu().data, gt_horizontaltop_img)
                        psnr_horizontaltop_val = angres_psnr(
                            gt_horizontaltop_img.cpu().data, fake_img[1].cpu().data)

                        ssim_horizontalbottom = pytorch_ssim.ssim(
                            fake_img[2][np.newaxis, :].cpu().data, gt_horizontalbottom_img)
                        psnr_horizontalbottom_val = angres_psnr(
                            gt_horizontalbottom_img.cpu().data, fake_img[2].cpu().data)

                        ssim_verticalleft = pytorch_ssim.ssim(
                            fake_img[3][np.newaxis, :].cpu().data, gt_verticalleft_img)
                        psnr_verticalleft_val = angres_psnr(
                            gt_verticalleft_img.cpu().data, fake_img[3].cpu().data)

                        ssim_verticalright = pytorch_ssim.ssim(
                            fake_img[4][np.newaxis, :].cpu().data, gt_verticalright_img)
                        psnr_verticalright_val = angres_psnr(
                            gt_verticalright_img.cpu().data, fake_img[4].cpu().data)
                ...
                j += 2
            i += 2
    print("\nMin center psnr is: ", min_center_psnr)
    ...
    print("\nMin hb psnr is: ", min_horizontalbottom_psnr)
    ...
    print("\nMin ht psnr is: ", min_horizontaltop_psnr)
    ...
    print("\nMin vl psnr is: ", min_verticalleft_psnr)
    ...
    print("\nMin vr psnr is: ", min_verticalright_psnr)
